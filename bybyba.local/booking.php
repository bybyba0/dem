...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$okItem) $errors['item_id'] = 'Выберите ' . $CONFIG['item_word'] . ' из списка.';

    // Дата в формате ДД.ММ.ГГГГ -> Y-m-d
    $sqlDate = null;
    if (preg_match('/^(\d{2})\.(\d{2})\.(\d{4})$/', $val['event_date'], $m)
        && checkdate((int)$m[2], (int)$m[1], (int)$m[3])) {
        $sqlDate = "{$m[3]}-{$m[2]}-{$m[1]}";
        if ($sqlDate < date('Y-m-d')) $errors['event_date'] = 'Дата не может быть в прошлом.';
    } else {
...
<section class="section">
  <div class="container narrow">
    <div class="section-head reveal">
      ...
      <p>Выберите <?= e($CONFIG['item_word']) ?>, удобную дату и способ оплаты.</p>
    </div>

    <form method="post" novalidate class="form card-form reveal">
      ...
      <div class="field-row">
        <div class="field <?= isset($errors['event_date']) ? 'has-error' : '' ?>">
          ...
          <input type="text" name="event_date" value="<?= e($val['event_date']) ?>"
                 placeholder="ДД.ММ.ГГГГ" inputmode="numeric" id="dateField" maxlength="10">
