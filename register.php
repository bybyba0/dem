...
<section class="auth-wrap">
  <div class="container auth-grid">
    <div class="auth-aside reveal">
      <h2>Присоединяйтесь к <?= e($CONFIG['brand']) ?></h2>
      <p><?= e($CONFIG['tagline']) ?></p>
      <ul class="auth-list">
        <li>✓ История ваших заявок в одном месте</li>
        <li>✓ Быстрое оформление и онлайн-оплата</li>
        <li>✓ Отзывы о проведённых мероприятиях</li>
      </ul>
    </div>

    <div class="auth-card reveal">
      <h1>Регистрация</h1>
      <p class="auth-sub">Заполните анкету — все поля обязательны.</p>
      <form method="post" novalidate class="form">
        <div class="field <?= isset($errors['login']) ? 'has-error' : '' ?>">
          <label>Логин</label>
          <input type="text" name="login" value="<?= e($old['login']) ?>" placeholder="например, ivanov2026" autocomplete="username">
        ...
        <div class="field <?= isset($errors['fio']) ? 'has-error' : '' ?>">
          <label>ФИО</label>
          <input type="text" name="fio" value="<?= e($old['fio']) ?>" placeholder="Иванов Иван Иванович">
        ...
        <div class="field-row">
          ...
          <div class="field <?= isset($errors['email']) ? 'has-error' : '' ?>">
            <label>E-mail</label>
            <input type="email" name="email" value="<?= e($old['email']) ?>" placeholder="you@mail.ru">
